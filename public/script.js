		...
		success: function (result) {
			if (Object.keys(result).length == 0) {
				...
				if (result.hasOwnProperty('Openness')) {
					...
					if(has5tweets=="yes"){
						let element = document.getElementById("alert");
						element.insertAdjacentHTML('afterbegin', "The user has less than the minimum number of chars, so personality statistics from IBM Watson cannot be shown.");
						//Put what u like to convey to user that WE CAN show top 5 tweets but we cant show personality insights
					}
					else{
						let element = document.getElementById("alert");
						element.insertAdjacentHTML('afterbegin', "The user has less than 5 tweets, so cannot show top 5 as well as less chars than required for personality statistics from IBM Watson cannot be shown.");
